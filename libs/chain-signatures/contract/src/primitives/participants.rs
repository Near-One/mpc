...
// todo: test that index is preserved during Migration
impl From<legacy_contract::primitives::Participants> for Participants {
    fn from(legacy_participants: legacy_contract::primitives::Participants) -> Participants {
        ...
        let next_id = ParticipantId(legacy_participants.next_id as u64);
        for (account_id, info) in &legacy_participants.participants {
            ...
            let id = ParticipantId(*id.unwrap() as u64);
        ...
        Participants {
            next_id: ParticipantId(legacy_participants.next_id as u64),
            participants,
            id_by_participant,
            participant_by_id,
        }
...
mod tests {
    ...
    fn test_participants() {
        ...
        for (idx, (account_id, info)) in expected.iter().enumerate() {
            ...
            assert_eq!(
                participants.account_id(&ParticipantId(idx as u64)).unwrap(),
                *account_id
            );
            assert_eq!(
                participants.id(account_id).unwrap(),
                ParticipantId(idx as u64)
            );
            assert!(participants.is_participant(account_id));
        }
        assert_eq!(participants.count(), n as u64);
        let expected: BTreeSet<ParticipantId> = (0..n).map(|i| ParticipantId(i as u64)).collect();
