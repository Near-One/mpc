...
mod tests {
    ...
    use crate::primitives::key_state::{AttemptId, KeyEventId, Keyset};
    ...
    fn test_resharing_contract_state_for(num_domains: usize) {
        ...
        let mut vote_result: Option<(Keyset, ThresholdParameters)> = None;
        ...
        for i in 0..num_domains {
            ...
            for _ in 0..20 {
                ...
                assert!(state
                    .vote_reshared(key_event, keyset, domain_registry)
                    .is_err());
            ...
            for bad_key_event in bad_key_events {
                for c in &candidates {
                    env.set_signer(c);
                    assert!(state
                        .vote_reshared(bad_key_event, keyset, domain_registry)
                        .is_err());
        ...
        let (keyset, _threshold_parameters) = vote_result.unwrap();

        assert_eq!(
            &running_state.parameters,
            state.resharing_key.proposed_parameters(),
        );
        assert_eq!(keyset.epoch_id, state.prospective_epoch_id());
        assert_eq!(keyset.domains, state.reshared_keys);
        assert_eq!(keyset.domains.len(), num_domains);
        assert_eq!(
